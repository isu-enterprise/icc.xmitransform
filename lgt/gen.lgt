...
:- category(listrenderable).
    ...
    renderaslist(_,_,""):-
        writef('ERROR: The list is empty, nothing to render!\n'),
        fail.
...
:- object(code_block, specializes(rootclass)).
    ...
    :- use_module(writef, [swritef/3, writef/1]).

    render(_):-
        writef("ERROR: Implement render/1 by a subclass!\n"),
        fail.

    :- public(simple_render/1).
    simple_render(Result):-
        findall(R, (::item(Item),
                    % writef:writef("item: %w\n",[Item]),
                    ::renderitem(Item, R)),
                Result).
    ...
    :- use_module(lists, [is_list/1, member/2]).

    render_to(List, Stream):-
        is_list(List),!,
        forall(member(X,List), render_to(X, Stream)).
...
:- category(namedtyped, extends(named)).
    ...
    renderitem(type(Type),String):-!,
        ::list_separator(Separator),
        swritef(String, '%w%w', [Separator, Type]).
...
:- object(class, specializes(code_block), imports([named])).
    ...
    render(Result):-
        ...
        (
            ...
            MethodList=[]),
        append(AttrList, MethodList, StringList),
        root::unindent,
...
:- object(mothur_attributes,
    ...
    multiple_selection_choices(property_parameter(Name,Query), [String, Choice]):-
        ...
        (nth0(Index,Options,Default) -> true;
    ...
    property_param_type_add(property_parameter(Name,Query), [Preamble,String]):-
        ::item(property_parameter(Name,Query)),
        ::mothur_property_data(Query,_Type,RType,_,DefaultValue,Description,Expert),
    ...
    param_type_constructor('Category', Name, Description, _, _Expert, String, []):-!,
        upcase_atom(Name,UNAME),
...
:- object(mothur_methods,
    ...
    renderitem(mothur_get_output_pattern(Query,_Class,PSM),
               ['',Override,Signature,
                Ifs,Super,E]):-!,
        ::override(Override),
    ...
    deliver_out_ports(Class, [Comment,Delivers]):-
        ...
        Module::current_query(_Query),  % TODO: What for this query?
        Class::item(attributes(Attributes)),
    ...
    process_inputs(Class, [Clear,
        ...
        Class::current_reference(module(Module)),
        Module::current_query(_Query), % TODO: see above TODO
        Class::item(attributes(Attributes)),
        ...
        findall(R,
                ...
                GetInputs),
        % (Class == o420 -> debugger::trace; true),
        % writef:writef("here\n"),
        findall(R,
                (
                    Attributes::item(property_parameter(Name,PQuery)),
                    % writef:writef("PP: %w\n",[property_parameter(Name,PQuery)]),
    ...
    render_get_property_paramenter(Name,Query, Attributes, [Load,Command]):-
        % (Name == taxonomy -> debugger::trace; true),
        Attributes::mothur_property_data(Query,
                               _Type,RType,JavaType,
                               _DefaultValue,_Description,_Expert),
        upcase_atom(Name,UNAME),
    ...
    render_get_property_paramenter0(_JavaType, Name, 'Category', _CCType, UNAME, [Load,Select]):-!,
        root::iswritef(Load, 'int %wIndex = getParameterAsInt(%w_LABEL);',
                       [Name, UNAME]),
        root::iswritef(Select, 'String %wValue = %w_CHOICES[%wIndex];',
                       [Name, UNAME, Name]).

    render_get_property_paramenter0(JavaType, Name, _RType, CCType, UNAME, Load):-
...
:- object(mothur_module,
    ...
    module_key_name(KeyName):-
        ...
        swritef(KeyName, 'mothur_%w_operator', [DName]).

    :- public(module_xml_class_name/1).
    module_xml_class_name(String):-
        ::module_class(_,Name),
        ::item(package(Package)),
        swritef(String, '%w.%w', [Package, Name]).
    ...
    module_icon_name(String):-
        ...
        Class::item(name(Name)),
        downcase_atom(Name, DName),
        swritef(String, 'icon-operator-%w.png', [DName]).
...
:- object(xml_block,
    ...
    attributes(Attributes):-
        ...
        ::append(attributes(Attributes)).
    attributes(Attributes):-
        writef('FATAL: Unknown structure for atttributes: %w',[Attributes]),
        fail.
